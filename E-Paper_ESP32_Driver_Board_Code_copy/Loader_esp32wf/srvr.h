...
const char *ssid = "404 network unavailable"; //"your ssid";
const char *password = "Albert0905!";   //"your password";

/* Static IP address Settings ------------------------------------------------*/
IPAddress staticIP(192, 168, 178, 126);
IPAddress gateway(192, 168, 178, 36);
...
bool Srvr__loop()
{
    ...
    while (client.available()) {
        ...
        if (Buff__bufInd > 4) {
            ...
            // Abruf über das Netzwerk starten
            if (Buff__signature(Buff__bufInd - 11, "/FETCH")) {
                Serial.println("Starte Bilddownload vom Server...");

                EPD_dispIndex = 46; // 4.26 inch B/W
                EPD_dispInit();
                EPD_downloadImage();

                isIndexPage = false; // Wichtig!
                break;
            }
