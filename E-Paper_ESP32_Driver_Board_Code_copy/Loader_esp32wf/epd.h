...
void EPD_downloadImage() {
    const char* imageUrl = "http://192.168.178.36:8000/bild.jpg";
    ...
    if (httpCode == 200) {
        ...
        Serial.println("Empfange Bilddaten von Server...");

        // Beginne mit dem Beschreiben des Displays
        EPD_dispInit(); // Initialisiert das Display
        ...
        // Fülle Buff__bufArr aus dem Stream (wie beim POST vorher)
        Buff__bufInd = 0;
        while (http.connected() && stream->available()) {
            Buff__bufArr[Buff__bufInd++] = stream->read();
            if (Buff__bufInd >= sizeof(Buff__bufArr)) break;
        }
        ...
        Serial.print("HTTP Fehler beim Abrufen des Bildes: ");
